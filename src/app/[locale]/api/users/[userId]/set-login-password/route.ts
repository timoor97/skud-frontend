import { setLoginPassword } from "@/services/usersService"
...
export const PATCH = async (req: Request, props: { params: Promise<{ userId: string }> }) => {
  try {
    ...
    const res = await setLoginPassword(body, Number(userId), locale)
    return NextResponse.json(res.data)
  } catch (error: unknown) {
    console.error("Error setting login password:", error);
